...
module HelpForEarlyYearsProviders
  class Application < Rails::Application
    ...
    config.feedback_url = ENV.fetch(
      'FEEDBACK_URL',
      config_for(:configuration)['feedback_url'] || '#FEEDBACK_env_var_missing',
    )
    config.signup_url = ENV.fetch('SIGNUP_URL', config_for(:configuration)['signup_url'] || '#SIGNUP_env_var_missing')
    config.tracking_id = ENV.fetch(
      'TRACKING_ID',
      config_for(:configuration)['tracking_id'] || '#TRACKING_ID_env_var_missing',
    )
